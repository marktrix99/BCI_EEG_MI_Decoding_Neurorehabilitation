function [fbcspw, model, mean_acc, std_acc] = ac_classification_model_fbcsp(folderName, csp_para, fs, start_sec, end_sec, class1, class2, ...
...
%% CSP Processing
fbcsp_results = struct();
band_names = fieldnames(filtered_data);
for b = 1:length(band_names)
    band = band_names{b};
    [fbcsp_results.(band).cspw, fbcsp_results.(band).F1, fbcsp_results.(band).F2] = ...
...
for b = 1:length(band_names)
    band = band_names{b};
    [F1_norm, F2_norm] = normalize_features(fbcsp_results.(band).F1, fbcsp_results.(band).F2);
