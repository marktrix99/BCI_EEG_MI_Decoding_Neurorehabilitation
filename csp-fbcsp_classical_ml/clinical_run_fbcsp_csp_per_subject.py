...
# %% [markdown]
# # Filter Bank Common Spatial Pattern and Common Spatial Patern Performanse Comparison on Acute Stroke Patients Dataset
# Attempt at implementing filter-bank common spatial filter (FBCSP) and CSP on an EEG motor imagery dataset containing 50 acute stroke patients
#   
# 


# %% [markdown]
# # Dataset (.mat and .edf data)
# 
...
# 
    ...
    classification_combinations = [
        ([1], [2], 'Left', 'Right'),      # Left vs Right MI
        ([1], [3], 'Left', 'Rest'),       # Left MI vs Rest
        ([2], [3], 'Right', 'Rest')      # Right MI vs Rest 
    ]
    ...
    def load_subject_data(subj, base_dir):
        data_path = os.path.join(base_dir, 'clinical data','sourcedata', subj, f'{subj}_task-motor-imagery_eeg.mat')
        ...
        try:
            ...
            # Extract rawdata and labels from the struct
            rawdata = eeg_struct['rawdata'][0,0]  # Shape: 40×33×4000
            labels = eeg_struct['label'][0,0]      # Shape: 40×1
    ...
    # Load all subject data from MAT files
    for i in range(sub_start, sub_end):
        ...
        # Extract events from channel 33
        fs = 500
        ...
        print(ori_data[subj]['etyp'])

    # %%
    # Check if labels are correctly assigned
    print("Left labels:", np.sum(etyp_list == 1))
    print("Right labels:", np.sum(etyp_list == 2))
    print("Rest labels:", np.sum(etyp_list == 3))
    ...
    for i in range(sub_start, sub_end):
        ...
        # Keep only first 30 EEG channels (remove last 3: EOG and marker)
        eeg_data = rawdata[:, :30, :]  # Now 40×30×4000
        
        # Reshape to continuous data for filtering (combine all trials)
        # Transpose to channels × trials × samples, then reshape
        continuous_eeg = eeg_data.transpose(1, 0, 2).reshape(30, -1)  # 30×160000
        
        # Store in mod_data structure
        mod_data[subj] = {
            'raw_EEG': continuous_eeg.T,  # Transpose to samples×channels (160000×30)
            'fs': 500,                   # Sampling rate
            'etyp': ori_data[subj]['etyp'],  # Copy event types
            'epos': ori_data[subj]['epos']   # Copy event positions
        }
    ...
    lowcut = 4
    highcut = 40
    fs = 500  # New Sampling rate after downsampling 125
    ...
    for first_class_code, second_class_code, label1, label2 in classification_combinations:
        ...
        for i in range(sub_start, sub_end):
            ...
            first_pos_all = ori_data[subj]['epos'][np.isin(ori_data[subj]['etyp'], first_class_code)]
            second_pos_all = ori_data[subj]['epos'][np.isin(ori_data[subj]['etyp'], second_class_code)]
            
            # Select trials depending on test_type
            if test_type == "full":
                mod_data[subj]['first_pos'] = first_pos_all[0:20]
                mod_data[subj]['second_pos'] = second_pos_all[0:20]
    ...
    # %%
        for i in range(sub_start, sub_end):
            ...
            for band in mod_data[subj]['EEG_filtered'].keys():
                ...
                # if min_trials == 0:
                #     print(f"⚠️ Skipping {subj} - {band} due to zero-length class.")
                #     continue
        ...
        def compute_S(avg_Cov, white):
            ...
            S = np.dot(np.dot(white.astype(np.float32), avg_Cov.astype(np.float32)), white.T.astype(np.float32))
    ...
    for first_class_code, second_class_code, label1, label2 in classification_combinations:
        ...
        for i in range(sub_start, sub_end):
            ...
            if test_type == "full":
                csp_data[subj]['first_pos'] = first_pos_all[0:20]  
                csp_data[subj]['second_pos'] = second_pos_all[0:20]
            elif test_type == "trial":
...
for subj_num in range(1, 51):  # For all 50 subjects
...
if all_results:
    ...
    results_df.to_excel("isolated_subject_results_clinical_data.xlsx")
